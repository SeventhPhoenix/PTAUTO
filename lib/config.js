(function() {
	...
	const options = program
		...
		.option('--ondemand-top-picks', 'for ondemand mode, only process the Top Picks category')
		.option('--ondemand-top-items [count]', 'for ondemand mode, only include top movie items per category (default count: 1)')
  ...
  Options:
    ...
    --ondemand-top-picks       : For ondemand mode, only process the Top Picks category
    --ondemand-top-items [count]: For ondemand mode, only include top movie items per category
	...
	const loadConfig = () => {
		const defaultconfig = {
			...
			xTvgUrl: false,
			ondemand: false,
			ondemandTopPicksOnly: false,
			ondemandTopItems: false,
			vlcopts: false,
			pipeopts: false
		}
