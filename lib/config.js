(function() {
	...
	const options = program
		...
		.option('--ondemand-all', 'include all ondemand VOD items instead of top 100')
  ...
  Options:
    ...
    --ondemand-all             : Include all ondemand VOD items instead of top 100
	...
	const loadConfig = () => {
		...
		for (const key of Object.keys(defaultconfig)) {
			...
			const optionValue = hasOwn(options, lck) ? options[lck] : options[key];
			const configValue = hasOwn(c, key) ? c[key] : c[lck];
			config[lck] = pickFirstDefined(optionValue, configValue, defaultconfig[key]);
		}
	}
	...
	const getMapping = () => {
		if (options.mapping) {
			...
			if (!region || !IP) {
				console.error('ERROR: --mapping must be in the format <region,IP>');
				process.exit(1);
			}
			const res = {};
			res[region] = IP;
