(function() {
	...
	const api = require('#lib/ptauto/api.js');
	...
	const serve = (config) => {
		server.addRoute('/{filename}', 'GET', async (req, res) => {
			try {
				...
				switch (ext) {
					case '.m3u8':
						const remoteAddress = req?.connection?.remoteAddress || res?.connection?.remoteAddress || '';
						const uuid = getUUID(config, remoteAddress);
						if (uuid) {
