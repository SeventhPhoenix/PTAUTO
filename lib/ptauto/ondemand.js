(function() {
	...
	const imagePath = (node) => asNonEmptyString(node?.path);

	const pickVodLogoPath = (vodItem) => {
		const candidates = [];
		const seen = new Set();
		const add = (path, priority) => {
			const clean = asNonEmptyString(path);
			if (!clean || seen.has(clean)) return;
			seen.add(clean);
			candidates.push({ path: clean, priority });
		};

		// Strong preference: explicit poster/cover/key-art fields.
		add(imagePath(vodItem?.coverImage), 100);
		add(imagePath(vodItem?.posterImage), 100);
		add(imagePath(vodItem?.poster), 100);
		add(imagePath(vodItem?.images?.cover), 100);
		add(imagePath(vodItem?.images?.poster), 100);
		add(imagePath(vodItem?.images?.keyArt), 100);
		add(imagePath(vodItem?.images?.portrait), 95);

		for (const entry of Array.isArray(vodItem?.images) ? vodItem.images : []) {
			const tag = `${entry?.type || ''} ${entry?.title || ''} ${entry?.kind || ''}`.toLowerCase();
			const path = imagePath(entry);
			if (!path) continue;
			if (/poster|cover|keyart|portrait/.test(tag)) add(path, 90);
			else if (/thumbnail|thumb|still|screenshot|tile/.test(tag)) add(path, 20);
			else add(path, 60);
		}

		for (const entry of Array.isArray(vodItem?.covers) ? vodItem.covers : []) {
			const tag = `${entry?.type || ''} ${entry?.title || ''} ${entry?.kind || ''}`.toLowerCase();
			const path = imagePath(entry);
			if (!path) continue;
			if (/poster|cover|keyart|portrait/.test(tag)) add(path, 90);
		...
		// Fallbacks: current screenshot-like/still sources.
		add(imagePath(vodItem?.image), 50);
		add(imagePath(vodItem?.thumbnail), 30);
		add(imagePath(vodItem?.stillImage), 25);
		add(imagePath(vodItem?.featuredImage), 10);
	...
	const generateM3U8 = async (config, region, bootData) => {
		...
		for (let j = 0; j < selectedVods.length; j++) {
			...
			const logoPath = pickVodLogoPath(vodItem);
			m3u8 += `#EXTINF:-1 tvg-id="${id}-${region}" tvg-logo="${logoPath}" tvg-chno="${tvgChno}" group-title="${groupTitle}", ${vodItem.name}\n${url}\n\n`;
			numVods++;
	...
	const generateXMLTV = async (config, region) => {
		...
		for (let i = 0; i < channelIds.length; i++) {
			...
			const channel = {
				...
				"icon": {
						"_attributes": {
							"src": utils.escapeHTML(pickVodLogoPath(entry))
						}
					}
				}
