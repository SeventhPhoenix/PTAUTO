(function() {
	...
	const CHANNEL_NAME_SUFFIXES = [' US Eastern Feed', ' Eastern Feed', ' US East', ' (US)', ' USA', ' USA Eastern', ',NY', ', NY', ' US', ',NY" group-title="Premium Channels"'];
	...
	const sanitizeExtinfLine = (line) => {
		...
		let beforeComma = out.slice(0, commaIdx);
		let afterComma = out.slice(commaIdx + 1);

		// Clean every quoted attribute value except URL-like fields.
		beforeComma = beforeComma.replace(/([a-z0-9-]+)="([^"]*)"/gi, (match, attr, value) => {
			if (['tvg-logo'].includes(attr.toLowerCase())) return match;
			return `${attr}="${stripSuffixes(value)}"`;
		});

		// Clean the display title after the comma.
