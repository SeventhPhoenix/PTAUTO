(function() {
	...
	const createLogoMapFromIptvOrg = async () => {
		const logoMap = new Map();
		const logoByIdMap = new Map();
		const res = await axios.get(IPTV_ORG_CHANNELS_URL);
		const channels = Array.isArray(res.data) ? res.data : [];

		for (const ch of channels) {
			if (!ch || !ch.logo) continue;
			const logo = String(ch.logo).trim();
			if (!logo) continue;
			if (ch.id && !logoByIdMap.has(ch.id)) logoByIdMap.set(ch.id, logo);
			if (ch.name) {
				const key = cleanChannelName(String(ch.name));
				if (key && !logoMap.has(key)) logoMap.set(key, logo);
			}
			if (ch.alt_names && Array.isArray(ch.alt_names)) {
				for (const altName of ch.alt_names) {
					if (!altName) continue;
					const key = cleanChannelName(String(altName));
					if (key && !logoMap.has(key)) logoMap.set(key, logo);
				}
			}
		}
